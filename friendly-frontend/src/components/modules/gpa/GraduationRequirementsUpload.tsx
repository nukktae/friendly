...
export const GraduationRequirementsUpload: React.FC<GraduationRequirementsUploadProps> = ({
  ...
  const uploadAndAnalyze = async () => {
    ...
    try {
      ...
      // Log FormData entries (TypeScript-safe)
      try {
        if ('entries' in formData && typeof (formData as any).entries === 'function') {
          const entries = Array.from((formData as any).entries() as IterableIterator<[string, string | File]>);
          console.log('[uploadAndAnalyze] FormData entries:', entries.map(([key, value]) => ({ key, valueType: typeof value })));
        } else {
          console.log('[uploadAndAnalyze] FormData entries: (unable to log - entries() not available)');
        }
      } catch (e) {
        console.log('[uploadAndAnalyze] FormData entries: (unable to log)');
      }
