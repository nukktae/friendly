...
router.post('/posts', checkSchoolVerification, postImageUpload.single('image'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      // In production, you would upload to cloud storage (Firebase Storage, S3, etc.)
      // For now, we'll store the relative path or you can implement cloud storage upload here
      // For this example, we'll store a relative path that the frontend can use
      imageUrl = `/uploads/posts/${path.basename(imagePath)}`;
      
      // TODO: Upload to cloud storage (Firebase Storage, AWS S3, etc.) and get public URL
      // For now, keeping the file on server - in production, upload to cloud storage
    }
...
router.patch('/posts/:postId', checkSchoolVerification, postImageUpload.single('image'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      // Set new image URL
      updates.imageUrl = `/uploads/posts/${path.basename(imagePath)}`;
